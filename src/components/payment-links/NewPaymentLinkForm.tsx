...
export function NewPaymentLinkForm({ orgSlug, publicBaseUrl }: { orgSlug: string; publicBaseUrl: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setCreatedUrl(`${publicBaseUrl}/give/${orgSlug}/link/${link.slug}`);
      setShowShare(true);
    } catch (err) {
      setError(errorMessage(err, "Could not create the payment link."));
      setBusy(false);
    }
  }
    ...
    <div className="max-w-2xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-ink-muted mt-1">
          This defines the giving page your supporters see when they open the link.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-surface border border-border p-6 sm:p-8 rounded-2xl flex flex-col gap-6">
        <div>
          ...
          <input id="pl-name" type="text" required value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. General donations" className={inputClass} />
        </div>
